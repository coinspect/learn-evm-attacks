...
/// @notice The PancakeSwap V3 Factory facilitates creation of PancakeSwap V3 pools and control over the
/// protocol fees
interface IPancakeV3Factory {
    ...
    /// @param tickSpacing The minimum number of ticks between initialized ticks for pools created with the
    /// given fee
    ...
    /// @dev A fee amount can never be removed, so this value should be hard coded or cached in the calling
    /// context @param fee The enabled fee, denominated in hundredths of a bip. Returns 0 in case of unenabled
    /// fee
    ...
    /// @notice Returns the pool address for a given pair of tokens and a fee, or address 0 if it does not
    /// exist @dev tokenA and tokenB may be passed in either token0/token1 or token1/token0 order
    ...
    /// @dev tokenA and tokenB may be passed in either order: token0/token1 or token1/token0. tickSpacing is
    /// retrieved from the fee. The call will revert if the pool already exists, the fee is invalid, or the
    /// token arguments
    /// are invalid.
    ...
    /// @param tickSpacing The spacing between ticks to be enforced for all pools created with the given fee
    /// amount
