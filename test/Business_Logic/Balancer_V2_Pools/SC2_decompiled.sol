...
contract BalancerExploitMath {
    ...
    function unknown524c9e20(
        ...
        uint256 manipulatedInvariant = _manipulateInvariant(
            normalizedWeight,
            adjustedBalances, 
            swapFeePercentage,
            indexOut
        ...
        uint256 virtualBalance = _calculateVirtualBalance(
            adjustedBalances,
            indexOut
        );
        
        // Core calculation that can trigger zero division in certain conditions
        uint256 weightedProduct = _computeWeightedProduct(
            adjustedBalances,
            indexOut,
            normalizedWeight,
            invariantRatio
        ...
        uint256 denominator = _computeDenominator(
            normalizedWeight,
            virtualBalance,
            swapFeePercentage
        );
        ...
        uint256 result = _divDown(
            _mulDown(weightedProduct, manipulatedInvariant),
            denominator
        );
    ...
    function _calculateInvariantRatio(
        ...
        for (uint256 i = 0; i < balances.length; i++) {
            if (i == 0) {
                ...
                uint256 weightedBalance = _powDown(
                    balances[i], 
                    normalizedWeight,
                    scalingFactors[i]
                );
    ...
    function _manipulateInvariant(
        ...
        for (uint256 i = 1; i < balances.length; i++) {
            uint256 adjustedBalance = _mulDown(
                balances[i],
                _complement(swapFee)
            );
        ...
        invariant = _divDown(
            _mulDown(invariant, weightedSum),
            BASIS_POINTS
        );
    ...
    function _computeWeightedProduct(
        ...
        for (uint256 i = 0; i < balances.length; i++) {
            if (i != excludeIndex) {
                uint256 weightedBalance = _powUp(
                    balances[i],
                    normalizedWeight,
                    product
                );
     ...
     */
    function _computeDenominator(
        uint256 normalizedWeight,
        uint256 virtualBalance,
        uint256 swapFee
    ) private pure returns (uint256) {
        uint256 feeAdjustedWeight = _mulDown(
            normalizedWeight,
            _complement(swapFee)
        );
    ...
    function _powDown(uint256 base, uint256 exp, uint256 precision) 
        private 
        pure 
        returns (uint256) 
    {
    ...
    function _powUp(uint256 base, uint256 exp, uint256 precision) 
        private 
        pure 
        returns (uint256) 
    {
