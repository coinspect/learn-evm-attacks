...
The attacker (which is suspected to be a rugpuller) drained swap pools because a miscalculation of the constant product in the swap function.
...
Ethereum Transaction Viewer: https://tx.eth.samczsun.com/binance/0x5a504fe72ef7fc76dfeb4d979e533af4e23fe37e90b5516186d5787893c37991
...
ATTACK:
The contract implementation modified the multiplied amount from 1_000 to 10_000 breaking the constant product check (x * y = k) (10_000 * 10_000 = 100MM instead of 1MM). Because of this, the balance0 
and/or the balance1 could be significantly smaller yet swap 100 times more. It is worth noting that the codebase was updated and the only modification introduced was the bug shown. The code before was:
...
Because of the nuances mentioned before, it is suspected that this attack was a type of "undercovered" rugpull.

The attacker funded the malicious contract with several tokens and then performed the swap on each pair draining the pools.
...
contract Exploit_Uranium is TestHarness, TokenBalanceTracker {
    ...
    function setUp() external {
        cheat.createSelectFork("bsc", 6947153); 
